...
class Image
{
    ...
    public function upfile($fname, $savepath=""){
        ...
        if  ($this -> upfile_size)  {
            if  ($file['size'] > $this->upfile_size)  {
                $this->erro_msg .= "file_size too big !\r\n"; 
                return false;
            }
        }
    
        if (!in_array($file['type'], $this->upfile_type)) {
            $this->erro_msg .= "Error:{$file['error']}\r\n type is error!\r\n"; 
            return false;
        }
    ...
    public function createImg($fname="",$savepath=""){
        ...
        if($bg_w < ($this->img_width+ceil($this->img_width/2)) || $bg_h < ($this->img_Height+ceil($this->img_Height/2))){
        	//height=h/(w/fixwidth);
        	//width=w/(h/fixheight);
       	 	$this->erro_msg .= "File is too small, can not to be watered!\r\n"; 
        	return false;
        }
    ...
    public function resizeImage($src_file, $savePath) {
        ...
        // 图像类型  exif_imagetype一般服务器不支持用getimagesize替换并取第三个返回值
...
$savePath = ROOT_PATH . "data/sysfile/";           		//上传图片目录
$img = new Image();
$file = $img->setsaveName($saveFileName)->upfile("file", $savePath); //将图片上传至$savePath
												
$thumbpath = ROOT_PATH . "data/sysfile/";             	//缩略图目录
$file_shrink = $img->setShrinkWidth(250) 				 //缩略图宽度
				   ->setShrinkHeight(80)				 //高度
				   ->setShrinkType("jpg")				 //缩略图类型
				   ->setsaveName("thumb_".$saveFileName) //缩略图名字
				   ->resizeImage($file,$thumbpath);      //生成缩略图方法

$waterlogo = ROOT_PATH . "/xyimg/xylogo.gif";
$waterpath = ROOT_PATH . "/photo/picWater/";         	 //加有水印图_图片目录
